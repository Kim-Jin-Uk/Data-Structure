...
class AVLTree {
    constructor() {
        ...
        const deleteNodeHelper = (root, item) => {
            ...
            if (item < root.item) root.left = deleteNodeHelper(root.left, item)
            else if (item > root.item) root.right = deleteNodeHelper(root.right, item)
            else {
            ...
            if (balanceFactor > 1) {
                if (this.getBalanceFactor(root.left) >= 0) return this.rightRotate(root)
                else {
            ...
            if (balanceFactor < -1) {
                if (this.getBalanceFactor(root.right) <= 0) return this.leftRotate(root)
                else {
